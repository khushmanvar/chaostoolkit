...
import (
	...
	"chaostoolkit/metrics"
	models "chaostoolkit/models"
	"chaostoolkit/utils"
)
...
// New returns a new instance of Chaos. It expects:
...
func New(client kubernetes.Interface, labels, annotations, kinds, namespaces, namespaceLabels labels.Selector, includedPodNames, excludedPodNames *regexp.Regexp, excludedWeekdays []time.Weekday, excludedTimesOfDay []models.TimePeriod, excludedDaysOfYear []time.Time, timezone *time.Location, minimumAge time.Duration, logger log.FieldLogger, dryRun bool, terminator models.Terminator, maxKill int, notifier models.Notifier, clientNamespaceScope string) *Chaos {
...
type Chaos struct {
	...
	ExcludedTimesOfDay []models.TimePeriod
...
func (c *Chaos) Run(ctx context.Context, next <-chan time.Time) {
	for {
...
func (c *Chaos) TerminateVictims(ctx context.Context) error {
	...
	// 	if tp.Includes(now) {
	// 		c.Logger.WithField("timeOfDay", now.Format(utils.Kitchen24)).Debug(msgTimeOfDayExcluded)
	// 		return nil
	// 	}
	// }
	...
	for _, d := range c.ExcludedDaysOfYear {
		if d.Day() == now.Day() && d.Month() == now.Month() {
			c.Logger.WithField("dayOfYear", now.Format(utils.YearDay)).Debug(msgDayOfYearExcluded)
			return nil
		}
	}
...
func (c *Chaos) Victims(ctx context.Context) ([]v1.Pod, error) {
	...
	pods = utils.RandomPodSubSlice(pods, c.MaxKill)
...
func (c *Chaos) Candidates(ctx context.Context) ([]v1.Pod, error) {
...
func (c *Chaos) DeletePod(ctx context.Context, victim v1.Pod) error {
...
func filterByOwnerReference(pods []v1.Pod) []v1.Pod {
	...
	for _, pods := range owners {
		filteredList = append(filteredList, utils.RandomPodSubSlice(pods, 1)...)
	}
